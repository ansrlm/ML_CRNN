...
def build_fpn_mask_graph(rois, feature_maps, image_meta,
    ...
    x = KL.TimeDistributed(KL.Dense(256))(x) # batch X bbox count X 256
    x = tf.identity(x, name='sh_feature')
    b_n, bbox_n = tf.get_shape(x)[0], tf.get_shape(x)[1]
    x = tf.reshape(x, (-1, 256)) # batch*bbox count X 256
    cell = tf.nn.rnn_cell.LSTMCell(256)
    sh_outputs, sh_state = tf.nn.dynamic_rnn(cell, x, sequence_length=25, dtype=tf.float32) # (batch*bbox_count) X 25 X 256
    #x = KL.TimeDistributed(KL.Conv2D(num_classes, (1, 1), strides=1, activation="sigmoid"),
    #                       name="mrcnn_mask")(x)
    sh_outputs = tf.reshape(sh_outputs, (-1, 256)) # (batch*bbox count*25) X 256
    sh_outputs = tf.layers.dense(sh_outputs, num_classes, acitvation=tf.nn.softmax) # (batch* bbox count* 25) X num_classes
    x = tf.reshape(sh_outputs, (b_n, bbox_n, 25, num_classes))
    return x
...
class MaskRCNN():
    ...
    def build(self, mode, config):
        ...
        if mode == "training":
            ...
            outputs = [mrcnn_bbox, mrcnn_mask,mask_loss]
